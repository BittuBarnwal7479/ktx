...
description: Connect ktx to PostgreSQL, Snowflake, Databricks, BigQuery, MySQL, ClickHouse, SQL Server, SQLite, or MongoDB.
---
...
|-------|----------|------------|-------------|
| `driver` | Yes | all connections | Connector driver such as `postgres`, `snowflake`, `databricks`, `bigquery`, `mysql`, `clickhouse`, `sqlserver`, `sqlite`, or `mongodb` |
